...
using System.Linq;
using System.Text;
using InterfaceGenerator.Shared;
...
{{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // //GeneratorLogging.LogMessage("[+] Added GenerateInterfaceAttribute to compilation");
        ...
        // //GeneratorLogging.LogMessage("[+] Starting incremental generation");
        // 👇 Inital extraction - the predicate and transform run very frequently
        // IncrementalValuesProvider<TypeDeclarationSyntax> typeDeclarations = context.SyntaxProvider
        //     .ForAttributeWithMetadataName(
        //         $"{Namespace}.{GenerateInterfaceAttributeName}",
        //         static (node, _) => IsSyntaxTargetForGeneration(node),
        //         static (ctx, _) => GetTypeForGeneration(ctx))
        //     .Where(static m => m is not null)!;

        //GeneratorLogging.LogMessage("[+] Extracted types for generation");

        //GeneratorLogging.LogMessage("[+] Registering source output");
        ...
        //GeneratorLogging.LogMessage("[+] Registered source output");
    ...
    private static TypeDeclarationSyntax? GetTypeForGeneration(GeneratorAttributeSyntaxContext context)
    {
    ...
    private static void Execute(Compilation compilation, ImmutableArray<TypeDeclarationSyntax> types, SourceProductionContext context)
    {
        try
        {
            ...
            foreach (TypeDeclarationSyntax? typeDeclaration in types)
            {
                ...
                //GeneratorLogging.LogMessage(sourceText.ToString());
